  ...
  try {
    if (subcommand === 'cluster') {
      ...
      await fetchUpstreamCommand({
        ...
        runId: values['run-id'],
        spec: values['filter-by-spec'] ?? values.spec,
    ...
    console.error(`Valid spec names (--spec, --filter-by-spec): ${VALID_SPECS.join(', ')}`);
