...
if (process.argv[1] && (process.argv[1] === import.meta.filename || process.argv[1].endsWith('run.ts') || process.argv[1].endsWith('run_wpt_node.ts'))) {
  ...
  (async () => {
    // Master fail-safe timeout: exit process after 45000ms to prevent orphaned background hangs
    setTimeout(() => {
      console.error("Runner timed out after 45000ms (self-termination fail-safe).");
      process.exit(1);
    }, 45000).unref();
